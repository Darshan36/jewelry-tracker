...
export function PaymentPanel({ entry, payments }: Props) {
  ...
  // For Plating/Plating: status semantics match Purchases (shop owes
  ...
  const onSubmit = async (data: FormOutput) => {
    setServerError(null);
    const result = await createPlatingPayment(data);
    if (!result.ok) {
  ...
  const handleReverse = (paymentId: string) => {
    startTransition(async () => {
      await softDeletePlatingPayment(paymentId);
      setReversingId(null);
      router.refresh();
    });
  };
    ...
    <div className="border border-outline-variant bg-surface-container-low p-4 space-y-4">
      ...
      {isFormOpen && (
        <form
          ...
          noValidate
        >
          <input type="hidden" {...register("platingEntryId")} />
