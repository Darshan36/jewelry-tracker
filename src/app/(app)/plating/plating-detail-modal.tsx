...
// Phase 10.6: PlatingDetailModal is now read-only. All mutation surfaces
...
// lives at /plating/[id]/edit; soft-delete is on the row's hover-action
// cluster.
//
...
//   - Notes
//   - Status chip
//   - One "Edit" link at the bottom that routes to /plating/[id]/edit
//
// Plating has no Returns workflow (see Phase 9 decision lineage), so
...
type Props = {
  ...
  entry: PlatingEntryForClient | null;
};
    ...
    <ResponsiveDialog open={open} onOpenChange={handleClose}>
      ...
      >
        ...
        <div className="space-y-5">
          ...
          {/* Phase 21a: party-linked plating entries reconcile on the
...
function PartyLedgerPointer({ partyId }: { partyId: string }) {
  // Phase 21a — party-linked plating entries reconcile on the party ledger.
  return (
    <div>
      <p className="font-display text-xs uppercase tracking-wider text-on-surface-variant mb-2">
        Payments
...
function ReadOnlyPaymentsList({
  ...
  payments: PlatingEntryForClient["payments"];
  paidAmount: number;
}) {
    ...
    <div>
      ...
      <div className="border border-outline-variant divide-y divide-outline-variant/40">
        {payments.map((p) => {
          ...
          // For Plating, REFUND = vendor refunded shop (money IN). Use
