...
export function PlatingDetailModal({
  open,
  onOpenChange,
  entry,
  onEdit,
  ...
  const handleDelete = () => {
    startTransition(async () => {
      await softDeletePlatingEntry(entry.id);
          ...
          {confirmingDelete ? (
            <div className="flex items-center gap-3">
              <p className="flex-1 text-sm text-on-surface">
                Delete plating entry? This can be undone by an admin.
              </p>
              <button
                type="button"
