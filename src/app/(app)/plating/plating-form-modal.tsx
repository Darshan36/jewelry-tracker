...
export function PlatingFormModal({ open, onOpenChange, entry, vendors }: Props) {
  ...
  // Upload a file to R2 attached to a plating entry. Two-stage flow
  ...
  async function uploadBillForEntry(entryId: string, file: File) {
    ...
    const attach = await attachBillToPlatingEntry(entryId, prep.billId);
  ...
  const onSubmit = async (data: FormOutput) => {
    ...
    // attached after upload via attachBillToPlatingEntry.
    ...
    if (pickedFile) {
      try {
        if (entry?.billId) {
          ...
          await detachBillFromPlatingEntry(savedEntry.id);
  ...
  const title = entry ? "Edit plating entry" : "Add plating entry";
