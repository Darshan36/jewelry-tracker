...
describe("platingLineItemSchema", () => {
  it("accepts a valid line item", () => {
    const result = platingLineItemSchema.safeParse(validLine());
    expect(result.success).toBe(true);
  });

  it("rejects empty materialDescription", () => {
    const result = platingLineItemSchema.safeParse({
      ...validLine(),
      materialDescription: "",
    });
...
describe("platingEntryInputSchema — top level", () => {
  it("accepts a valid input with one line item", () => {
    const result = platingEntryInputSchema.safeParse(validInput());
    expect(result.success).toBe(true);
  });

  it("rejects an empty lineItems array (min 1)", () => {
    const result = platingEntryInputSchema.safeParse({
      ...validInput(),
      lineItems: [],
    });
    expect(result.success).toBe(false);
  ...
  it("defaults discount to 0 when omitted", () => {
    ...
    const result = platingEntryInputSchema.safeParse(rest);
    expect(result.success).toBe(true);
