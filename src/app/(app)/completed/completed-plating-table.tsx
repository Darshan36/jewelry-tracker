...
// Phase 19 — read-only completed-plating table.
// Structural mirror of completed-casting-table.tsx.
...
export function CompletedPlatingTable({ entries, onRowClick }: Props) {
  return (
    <ResponsiveTable
      desktopTable={
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {entries.map((e) => (
                <tr
                  key={e.id}
                  data-testid={`completed-plating-row-${e.id}`}
        ...
        <>
          {entries.map((e) => (
            <MobileCard
              ...
              clickable
              onClick={() => onRowClick(e.id)}
              data-testid={`completed-plating-card-${e.id}`}
            >
              <MobileCardHeader>
                <MobileCardTitle>
