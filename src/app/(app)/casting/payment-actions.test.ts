...
import type { CastingPayment, Role } from "@/generated/prisma";
...
describe("createCastingPayment — PAYMENT happy path", () => {
  it("inserts a payment in BigInt paise (₹1,000 → 100000n)", async () => {
    ...
    vi.mocked(prisma.castingPayment.create).mockResolvedValue({
      id: "payment-new",
    } as unknown as CastingPayment);

    const result = await createCastingPayment(validPayment());

    expect(result.ok).toBe(true);
    const call = vi.mocked(prisma.castingPayment.create).mock.calls[0][0];
  ...
  it("rejects PAYMENT exceeding remaining balance with formatted error", async () => {
    ...
    vi.mocked(prisma.castingEntry.findUnique).mockResolvedValueOnce(
      makeEntry(200000n, [{ amount: 150000n, type: "PAYMENT" }]),
    );

    const result = await createCastingPayment(validPayment({ amount: 1000 }));

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.errors.amount?.[0]).toMatch(/Owed to vendor: ₹500\.00/);
    }
    expect(prisma.castingPayment.create).not.toHaveBeenCalled();
  });
  ...
  it("returns ok=false when the parent entry does not exist", async () => {
    vi.mocked(prisma.castingEntry.findUnique).mockResolvedValueOnce(null);

    const result = await createCastingPayment(validPayment());

    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.errors.castingEntryId).toBeDefined();
  });
});

describe("createCastingPayment — REFUND handling", () => {
  it("accepts a REFUND up to the net paid amount", async () => {
    ...
    const result = await createCastingPayment(validPayment({ amount: 500, type: "REFUND" }));
  ...
  it("rejects a REFUND exceeding the net paid amount", async () => {
    ...
    const result = await createCastingPayment(validPayment({ amount: 1500, type: "REFUND" }));
  ...
  it("net paid is PAYMENT minus REFUND — previous refunds reduce headroom", async () => {
    ...
    const result = await createCastingPayment(validPayment({ amount: 1001, type: "REFUND" }));
...
describe("softDeleteCastingPayment", () => {
  it("sets deletedAt and returns the parent castingEntryId", async () => {
    ...
    const result = await softDeleteCastingPayment("payment-1");

    expect(result.ok).toBe(true);
    if (result.ok) expect(result.castingEntryId).toBe("entry-1");
    const call = vi.mocked(prisma.castingPayment.update).mock.calls[0][0];
...
describe.each(ROLE_MATRIX)(
  "createCastingPayment role access — %s",
  (role, allowed) => {
    it(allowed ? `allows ${role}` : `denies ${role} (Forbidden)`, async () => {
      if (allowed) {
        ...
        const r = await createCastingPayment(validPayment());
        expect(r.ok).toBe(true);
      } else {
        vi.mocked(requireRole).mockRejectedValueOnce(new Error("Forbidden"));
        await expect(createCastingPayment(validPayment())).rejects.toThrow("Forbidden");
        expect(prisma.castingPayment.create).not.toHaveBeenCalled();
      }
    });
  },
