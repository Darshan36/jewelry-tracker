// Component tests for the casting form modal — multi-line useFieldArray
...
import { CastingFormModal } from "./casting-form-modal";
import type { VendorOption } from "./party-picker";

const vendors: VendorOption[] = [
  { id: "vendor-1", name: "Mahesh Casting Works", phone: "9876543210" },
];
...
describe("CastingFormModal — initial render", () => {
  ...
  it("seeds the date input to today (YYYY-MM-DD)", () => {
    ...
    const dateInput = document.getElementById("casting-date") as HTMLInputElement;
  ...
  it("renders a vendor picker with the supplied vendors as autocomplete options", async () => {
    ...
    const picker = document.getElementById("casting-party-name") as HTMLInputElement;
    await user.click(picker);
    expect(screen.getByText("Mahesh Casting Works")).toBeInTheDocument();
  });
});

describe("CastingFormModal — multi-line useFieldArray", () => {
...
describe("CastingFormModal — live line-total preview", () => {
  ...
  it("flags discount-exceeds-subtotal at the live preview layer", async () => {
    ...
    const discount = document.getElementById("casting-discount") as HTMLInputElement;
...
describe("CastingFormModal — submit flow without bill", () => {
  it("calls createCastingEntry on save when entry is undefined (create mode)", async () => {
    const user = userEvent.setup();
    ...
    const partyName = document.getElementById("casting-party-name") as HTMLInputElement;
  ...
  it("calls updateCastingEntry on save when entry prop is supplied (edit mode)", async () => {
    const user = userEvent.setup();
