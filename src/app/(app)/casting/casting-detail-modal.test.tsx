...
describe("CastingDetailModal — Phase 10.6 read-only contract", () => {
  ...
  it("Delete is always-visible (no hover gate) and confirms before calling softDeleteCastingEntry (Phase 22.1)", async () => {
    ...
    render(
      <CastingDetailModal
        ...
        entry={makeEntry({ id: "c-del" })}
      />,
    );

    const del = screen.getByTestId("modal-delete-casting-c-del");
    expect(del).toBeVisible();

    await user.click(del);
    expect(softDeleteCastingEntry).not.toHaveBeenCalled();

    await user.click(screen.getByTestId("modal-confirm-delete-casting-c-del"));
    await waitFor(() => {
      expect(softDeleteCastingEntry).toHaveBeenCalledWith("c-del");
    });
