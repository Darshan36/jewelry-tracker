...
//       prepareUpload → R2 PUT → confirmUpload → attachAttachmentToCastingEntry
...
export function CastingForm({ mode, entry, vendors }: Props) {
  ...
  const onSubmit = async (data: FormOutput) => {
    ...
    // The FK gets attached after upload via attachAttachmentToCastingEntry.
    ...
    if (pickedBillFile) {
      try {
        ...
        const attach = await attachAttachmentToCastingEntry(savedEntryId, prep.attachmentId);
