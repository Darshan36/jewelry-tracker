...
//   - Currency fields (rate, discount) stay as `number` rupees here. Do NOT
//     `.transform(v => BigInt(...))` — the client form would emit BigInt and
//     the server-action re-parse would reject it ("Expected number, received
//     bigint"). Conversion to BigInt paise happens in actions.ts at the
//     Prisma boundary.
