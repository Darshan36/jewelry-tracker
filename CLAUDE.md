...
- **Auth:** Auth.js v5 beta with Credentials provider, **JWT strategy** (stateless JWE cookies, 30-day expiry), **bcryptjs** cost 12, `Role` enum (`ADMIN | PURCHASE_DEPT | LABOUR_MGMT | CASTING_PLATING_MGMT`) in Prisma schema. Entry point: `src/lib/auth.ts` exporting `{ auth, handlers, signIn, signOut }`. The Credentials provider's `authorize` callback delegates to `src/lib/authorize-credentials.ts` so unit tests can pin deactivation/bcrypt/null branches. **Active-status gate at login**: `authorize` rejects users whose `deletedAt IS NOT NULL` BEFORE bcrypt — a deactivated user with the correct password fails. **Per-request session validation (Phase 21.fix)**: the JWT callback in `src/lib/auth-callbacks.ts` re-reads the user from the DB on every request — if deactivated or gone, returns `null` (Auth.js invalidates the cookie); if role changed, silently refreshes `token.role` (no re-login). DB error in the callback → KEEP the existing token (FAIL-SAFE; prevents DB-blip mass logout). See §6 conventions for the asymmetric error-handling pattern.
- **RBAC:** Three-layer access on every gated route — (1) `src/proxy.ts` URL-prefix `ROUTE_ROLES` (reads `req.auth.user.role` from the per-request-validated JWT); (2) page server-component redirect via `canX(role)` helpers in `src/lib/role-access.ts`; (3) every server action calls `await requireRole([...allowedRoles])` as its first await. **`requireSession` / `requireRole` independently re-read the user from the DB (Phase 21.fix)** — reject deactivated users with `Unauthorized`, use FRESH DB role for the authorization decision (not `session.user.role`, which is no longer source of truth). DB error in the guard → throws `Auth check failed` (FAIL-CLOSED; better to bounce one user than allow a stale-token write). Per-role sidebar filtering in `src/app/(app)/sidebar.tsx`. **Defense-in-depth posture**: proxy URL gate is SINGLE-LAYER (jwt callback only — reads); action layer is DOUBLE-LAYER (jwt + independent requireRole DB check — writes are independently safe even if jwt regressed).
...
- **Role gates on every server action.** Every server action calls `await requireRole([...allowedRoles])` as its first await. The list is per-action and declares which roles can invoke it — reading the action's first line tells you the access matrix at a glance. **Post-21.fix, the guard does an independent PK DB lookup** on `users` to confirm active status + read the fresh role — `session.user.role` is no longer trusted for authorization decisions. The fresh DB role is what gets compared to `allowedRoles`. Adds ~5ms per action call; negligible at workshop scale.
- **Auth re-validates role + active-status against the DB on every request (Phase 21.fix).** The pre-fix JWT callback baked role/identity into the token at login and never re-read it — role changes and deactivations didn't propagate to live sessions for up to 30 days (session maxAge). Fix is two independent layers: (1) `jwtCallback` in `src/lib/auth-callbacks.ts` does a `prisma.user.findUnique` on every request — deactivated/gone → return `null` (Auth.js invalidates the session cookie); role changed → silent `token.role` refresh (no re-login); (2) `requireSession` / `requireRole` in `src/lib/auth-guards.ts` do the SAME independent lookup and use the fresh DB role for authorization. **Asymmetric error handling — both intentional, both pinned by tests + live rehearsal**: the jwt callback FAILS SAFE on DB error (keeps existing token — a DB blip must not log every user out simultaneously); the guards FAIL CLOSED on DB error (throw "Auth check failed" — actions write data; better to bounce one user than allow a stale-token write through during a blip). The asymmetry is because jwt runs on every request (reads + writes) while requireRole runs only on writes. **Defense-in-depth posture** (locked, see Phase 21.fix in §7): proxy URL gate is SINGLE-LAYER (jwt callback only — reads are protected by one layer, acceptable); action layer is DOUBLE-LAYER (jwt + independent guard re-check — writes are independently safe even if jwt regressed). Do NOT add a third DB lookup to the proxy — the jwt callback already covers it on every page-nav.
...
- **`/ledger` owner list is tabbed by category for ADMIN (Phase 21c.1.1).** Five tabs — All (default) / Sales / Purchase / Casting-Plating / Karigar — filter the unified owner list by category membership. Mirrors the employees-table `FilterPill` pattern (`role="radio"`, `h-8 px-3 uppercase`, `bg-primary` when active). Tab bar visibility is **data-shape-derived**, NOT role-string-checked: `showTabs = boxes.length > 1`. ADMIN sees 4 boxes → tab bar renders; scoped roles see 1 box → tab bar hidden (their single list is exactly the All-view). **Per-tab balance projection**: under a category tab, each owner row shows that category's slice (`owner.slices[k]`), not the full balance. Dual-role parties appear under each role's tab with the role-specific amount. Karigars are always-included (incl. zero-balance) under the Karigar tab — always-available-surface pattern preserved across the tab filter. Walk-in section is tab-state-independent (stays standalone below the owner list).
- **Per-category slices on owner rows feed BOTH the tab filter AND the box totals (Phase 21c.1.1).** `listLedgerHome` was refactored from 4 separate per-box loops into a SINGLE per-owner loop: each category's slice is computed once and pushed into both the owner's `slices: Partial<Record<LedgerBoxKey, number>>` map AND the matching box accumulator. Structural invariant by construction: `Σ owners[].slices[k] ≡ boxes[k].total` to the paise. The tab and the box can't disagree because they're literally the same arithmetic, applied once. Slice presence rule: party slices set ONLY when non-zero (matches the box count rule); karigar slice set for EVERY karigar including zero (matches the always-available-surface pattern — zero karigars still appear under Karigar tab).
- **Dashboard renders individual clickable category boxes via `LedgerBoxesGrid` (Phase 21c.1.1 — supersedes 21c.1's `GoToLedgerCard`).** Each role's dashboard shows N per-category clickable boxes — ADMIN gets 4 (Receivables / Purchase / Casting+Plating / Karigar); scoped roles get 1 (their category). Each box links to `/ledger?tab=<slug>` via `ledgerHrefForBox()`. The 21c.1 single Go-to-Ledger card was the right consolidation at the time (killed dashboard/payables drift by collapsing duplicates) but lost the per-category breakdown the workshop owner wanted at a glance — 21c.1.1 brings it back as navigation entry points without re-introducing drift. Pre-21c per-source cards + top-3 party lists stay gone. Component lives in `src/app/(app)/dashboard/ledger-boxes-grid.tsx` (extracted to a standalone file so it's importable + unit-testable). Tx-count + monthly-aggregate + bills-stored + labour-section cards still stay — period facts, not duplicated balance state.
- **Drift-proof same-source invariant for dashboard vs /ledger (Phase 21c.1 → 21c.1.1).** Both `dashboard/page.tsx`'s `<LedgerBoxesGrid boxes={home.boxes} />` and `ledger/page.tsx`'s `<LedgerClient boxes={home.boxes} ... />` consume the SAME `LedgerBox[]` array from ONE `listLedgerHome(role)` call per page render. `LedgerBoxesGrid` and `LedgerClient` both render `box.total` verbatim — zero arithmetic, zero transformation. Pinned by the `LedgerBoxesGrid` "displays the box.total VERBATIM" component test. The founding-bug of the entire 21a→21c arc (dashboard/payables drift) is now impossible without a deliberate code-level change to break the invariant. Live-proven on prod walkthrough: dashboard box total === /ledger box total per category, to the paise, every time.
- **`?tab=<slug>` deep-link to /ledger (Phase 21c.1.1 — one-way only).** `/ledger?tab=<slug>` pre-selects that tab on page load. Slug vocabulary is shared between encoder + parser, both in `src/lib/ledger-home.ts`: `LEDGER_TAB_SLUG_BY_BOX` constant + `ledgerHrefForBox(key)` encoder + `parseLedgerTabSlug(raw)` parser. Slugs are user-friendly (`sales`, `purchase`, `casting-plating`, `karigar`); missing or invalid → `"all"` (defensive). Round-trip pinned by tests. The /ledger page server component reads `searchParams.tab` via `await searchParams`, passes the parsed value as `initialTab` to `LedgerClient`. **Client-side `effectiveTab` clamp** (`showTabs ? tab : "all"`) ensures a scoped role with a stray `?tab=` param still renders their single list — no way to be stranded by a URL. **One-way only**: clicking tabs inside /ledger does NOT update the URL (no `router.replace` on tab change). This is a deliberate scope cut — adding tab→URL sync would let browser back/forward preserve tab state but adds reactive plumbing; deferred as a minor follow-up until real-use demand surfaces.
...
### Phase 21c.1.1 — Category tabs on /ledger + dashboard per-category boxes

Small UI add-on to 21c.1, BEFORE the 21c.2 cleanup. Two surfaces, one source: category tabs filter the /ledger owner list by category for ADMIN; the dashboard's single Go-to-Ledger card is replaced with N individual clickable category boxes (one per box the role can see). Each dashboard box deep-links into /ledger via `?tab=<slug>` pre-selecting the matching tab. See §6 conventions for the full set of new entries: "`/ledger` owner list is tabbed by category", "Per-category slices on owner rows feed BOTH the tab filter AND the box totals", "Dashboard renders individual clickable category boxes via `LedgerBoxesGrid`", "Drift-proof same-source invariant for dashboard vs /ledger", "`?tab=<slug>` deep-link to /ledger (one-way only)".

**Three things land**:

**(1) `listLedgerHome` refactored to single per-owner loop.** Previously 4 separate per-box loops over `parties` (one per category) + 2 separate per-owner loops. Now ONE per-owner loop computes each category's slice ONCE — the slice value feeds BOTH `owner.slices[key]` AND the matching box accumulator. This is the structural fix that makes the tab/box reconciliation invariant by-construction: `Σ owners[].slices[k] ≡ boxes[k].total` to the paise, proven in rehearsal across the full owner set including baseline residual data.

**(2) `/ledger` owner list tabbed for ADMIN.** 5 tabs (All / Sales / Purchase / Casting-Plating / Karigar) via the same `FilterPill` pattern as employees-table. `showTabs = boxes.length > 1` (data-shape derived, not role string) keeps scoped roles tab-free. Per-tab balance projection swaps `owner.balance` for `owner.slices[tab]` on each rendered row. Dual-role parties appear under each role's tab with the role-specific slice. Karigar tab includes zero-balance karigars (always-available-surface preserved). Walk-in section is tab-state-independent.

**(3) Dashboard per-category boxes.** `<LedgerBoxesGrid boxes={home.boxes} />` replaces the single `<GoToLedgerCard>`. ADMIN gets 4 clickable boxes; scoped roles get 1. Each box links to `/ledger?tab=<slug>` via `ledgerHrefForBox()`. The dashboard and /ledger consume the SAME `home.boxes` array from one `listLedgerHome(role)` call; both render `box.total` verbatim (pinned by component test). Same source, two display sites, zero recomputation.

**`?tab=<slug>` URL deep-link** is one-way: dashboard → /ledger pre-selects. Slug vocabulary in `src/lib/ledger-home.ts` (`LEDGER_TAB_SLUG_BY_BOX` + `ledgerHrefForBox` encoder + `parseLedgerTabSlug` parser). Invalid/missing → "all". Scoped role with stray `?tab=` param → client-side `effectiveTab` clamps to "all" so they always render their single list. Tab changes inside /ledger do NOT update the URL (the minor follow-up).

**Zero schema, zero migration, zero data movement.** Pure read-side + UI + one new server-page prop.

**Dual-role 'two questions, two correct answers'**: a dual-role party (customer + supplier + casting vendor) with SALE + PURCHASE + CASTING + MANUAL_PAYMENT correctly shows four different per-tab numbers — Sales-tab shows the receivables slice (incl. MANUAL_PAYMENT), Purchase-tab the purchase slice (excl. MANUAL_PAYMENT), Casting/Plating-tab the casting slice, All-tab the full net balance. Each is the correct answer to its tab's question; the per-box reconciliation invariant `Σ slices[k] ≡ box[k].total` still holds across the full owner set including the dual-role party. Not a reconciliation failure — different questions, different answers, integrity preserved.

**Verification**:
- `tsc --noEmit` clean.
- 1645 → 1676 tests PASS (+31 net): 7 slice tests + 1 tab/box reconciliation invariant + 6 URL slug + 4 round-trip in `ledger-home.test.ts`; 7 tab tests + 3 initialTab tests in `ledger-client.test.tsx`; NEW 9-test `dashboard/ledger-boxes-grid.test.tsx` covering per-role box count, hrefs, drift-proof VERBATIM rendering, credit/zero/pluralisation.
- Neon rehearsal 75/75 PASS — delta reconciliation per box (Δrecv +₹41k, Δpurch +₹64k, Δcp +₹73k, Δkar +₹1k) + structural invariant (Σ slices === box.total exact) + drift-proof (idempotent listLedgerHome) + dual-role 4-number proof + URL slug round-trip + scoped-role data-layer no-leak.
- **Prod walkthrough 51/51 PASS** on commit `75702e7` (deployment `dpl_B2ztEXjFocLuHTLTjZ53awvv3wko`), marker `__phase21c1_1_walk_1779538796495`. **Live drift-proof confirmed exact**: dashboard box totals === /ledger box totals to the paise (receivables 1,200,000p === 1,200,000p; purchase 400,000p === 400,000p; casting/plating 0p === 0p; karigar 500,000p === 500,000p). Live no-leak confirmed: PURCHASE_DEPT saw 1 dashboard box + no /ledger tab bar + zero karigars in owner list + marker karigar NOT visible to them; LABOUR_MGMT mirror result (1 box + no tabs + zero parties + marker karigar IS visible). URL deep-link: `?tab=sales`/`karigar`/`invalid` all land correctly. Mobile 390×844: dashboard 4-box grid + /ledger tab bar both render without horizontal scroll. Regression: /payables, /receivables, /completed all 200 + URL unredirected.

**Prod-state observation**: the 2 orphan party-ledger rows (KNOWN_GAPS cascade bug, 21c.2 cleanup target) are still present. They don't affect /ledger or dashboard rollups (the `JOIN parties + p.deletedAt IS NULL` filters them out at the helper layer). Hitesh's prod party-side ledger remains empty today; the marker dual-role party introduced for the walkthrough confirmed the dual-role-in-both-tabs flow end-to-end live, then was cleaned up.

**Lesson — drift-proof via three independent guards is the strongest form**:
- (a) Single source — `listLedgerHome` is called once per page render, the same array reaches both display surfaces.
- (b) Idempotent — two successive calls return identical boxes (rehearsal proven).
- (c) Verbatim rendering — pinned by component test that `LedgerBoxesGrid` and `LedgerClient` both show `box.total` without transformation.
Any one of these alone leaves a possible drift path; all three together make the bug class impossible without a deliberate code change to break ALL THREE.

### Phase 21.fix — Stale-JWT security fix (top priority, ahead of all 21c work)

Resolves an access-control bug discovered after 21c.1.1 shipped: role changes and deactivations didn't propagate to live sessions. A demoted user kept their old (higher) role; a deactivated user kept full access — both for up to 30 days (the session `maxAge`).

**Root cause** (textbook stale-JWT, diagnosed at Checkpoint 0): the pre-fix `jwt` callback only ran its body on initial sign-in (`if (user) { … }` branch). On every subsequent request, `user` was undefined → branch skipped → token returned unchanged → role + identity frozen for the token's lifetime. `deletedAt` was never on the token at all. All three defense-in-depth layers (proxy URL gate, page server-component redirect, server-action requireRole) read from `session.user.role` derived from this stale token → all three failed open together. DB writes were always correct; the bug was purely on the session/read side.

**Fix — two independent layers**:

**(1) jwt callback re-reads the user from the DB per request.** New `src/lib/auth-callbacks.ts` (extracted for unit testing, same pattern as Phase 16's `authorize-credentials.ts`). On initial sign-in: seed token (unchanged). On every subsequent request: `prisma.user.findUnique` by `token.id`; deactivated/gone → return `null` (Auth.js invalidates the session cookie); role changed → silent `token.role` refresh. **DB error → keep existing token (FAIL-SAFE)** — pinned by unit test + live rehearsal. The fail-safe is what prevents a self-inflicted outage: returning null on a transient DB blip would log every user out simultaneously.

**(2) `requireSession` / `requireRole` independently re-fetch.** `src/lib/auth-guards.ts` rewritten — both guards do their OWN `prisma.user.findUnique` after reading the session. Throws `Unauthorized` if user gone/deactivated. `requireRole` uses the FRESH DB role for the authorization decision; `session.user.role` is no longer trusted. **DB error → throws "Auth check failed" (FAIL-CLOSED)** — better to bounce one user with an error toast than allow a stale-token write during a DB blip.

**Asymmetric error handling rationale** (jwt fail-safe vs guard fail-closed):
- jwt runs on EVERY request (reads + writes). Failing open on DB error here would log every user out on any DB hiccup → mass-logout DoS. Fail safe.
- requireRole runs ONLY on writes (server actions). Failing open on DB error here would let a deactivated user complete writes during a blip. Fail closed.

**Defense-in-depth posture** (locked architectural choice):
- **Proxy URL gate** (page reads): SINGLE-LAYER (jwt callback only). A regressed proxy would fail-open on page LOADING only.
- **Action/write layer**: DOUBLE-LAYER (jwt callback + independent requireRole DB check). Writes are independently safe even if the jwt callback regressed. Pinned by unit test "STALE TOKEN role — token says ADMIN, DB says PURCHASE_DEPT → Forbidden when ADMIN required".

The single-layer-for-reads + double-layer-for-writes asymmetry is intentional. Adding a DB check to the proxy would be a third per-page-nav DB read; redundant given the jwt callback covers it already.

**Per-request cost**: +1 indexed PK `findUnique` per request (~5ms via pooler). At workshop scale (1-3 concurrent users), negligible.

**Verification**:
- `tsc --noEmit` clean.
- 1676 → 1704 tests PASS (+28 net): NEW `auth-callbacks.test.ts` (14: sign-in seed × 2 / DB-confirms-active / role refresh × 2 / deactivated → null × 2 / **FAIL-SAFE on DB error × 3** / defensive paths / session callback × 3); NEW `auth-guards.test.ts` (15: requireSession 6 incl. **FAIL-CLOSED on DB error** + requireRole 9 incl. **STALE TOKEN role test**). Existing `authorize-credentials.test.ts` (Phase 16) unchanged.
- **Neon rehearsal 17/17 PASS** against live dev server + real cookies: legitimate active uninterrupted (4); DEMOTE live ADMIN→PURCHASE_DEPT loses access next request no re-login (1); PROMOTE live PURCHASE_DEPT→ADMIN gains access next request no re-login (2); **DEACTIVATE live → cookie killed end-to-end** — Auth.js v5 actually invalidates the session when jwtCallback returns null (3, the framework-dependent case); fresh login deactivated rejected + reactivated accepted (5); **TRANSIENT DB ERROR → token preserved with all fields incl. arbitrary custom field** (7).
- **Prod walkthrough 21/21 PASS** on commit `c075786` (deployment `dpl_H3AqJMT9crR7Q9oxyzRyjRY8uZVs`), marker `__phase21fix_walk_1779544682812`. Sequence executed in heightened-discipline order: ADMIN-not-locked-out FIRST (separately from full walkthrough, with rollback staged) → ADMIN OK → full walkthrough → cleanup. **Live drift-class proof on PROD**: deactivation invalidates the cookie via Auth.js's null-return semantics; promotion + demotion both take effect within one request; transient DB error against PROD Supabase preserves the token (id/role/iat/custom all retained); 21c.1.1 surfaces (dashboard boxes + /ledger tabs) regression-clean. Marker test user cleaned up.

**Rollback discipline**:
- Pre-fix HEAD identified BEFORE deploy: `a0c15f8`.
- Revert command staged BEFORE deploy: `git revert c075786 --no-edit && git push` (~3 min total rollback time).
- Admin-not-locked-out check run FIRST, before the full walkthrough — would have triggered immediate rollback if admin couldn't access the app.
- Rollback not needed; admin access confirmed within seconds of deploy READY.

**Lesson — auth deploys require their own discipline**: this was the first deploy where rollback was staged + the admin access check was sequenced ahead of the full walkthrough. Both choices proved valuable framing even though rollback wasn't needed. For any future change that touches every request (auth, middleware, the prisma client itself), follow the same pattern: identify rollback target before deploy, prove the deployer hasn't locked themselves out before doing the rest of the walkthrough.
