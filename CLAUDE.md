...
- **UX migration (Phase 10 — Sales only):** introduced the form-as-page architecture (`/sales/new`, `/sales/[id]/edit`) and inline-action-modal patterns (Pay / Bill / Return icon buttons in the row Actions column, dispatching to shared `PaymentActionModal` / `BillActionModal` / `ReturnActionModal`). Sales detail modal is **read-only** — all mutations route through inline buttons or the edit page. Bill upload preview via browser-native `URL.createObjectURL` (image `<img>`, PDF `<embed>`). **Purchases, casting, and plating remain on the previous modal-everything UX** until Phase 10.5 mirrors the pattern.
- **Hosting:** Vercel
- **Package manager:** npm
...
- **Forms as pages (Sales, Phase 10).** `/sales/new` and `/sales/[id]/edit` are dedicated server-component routes. The form lives in `sale-form.tsx` as a standalone client component (RHF + `useFieldArray` + `zodResolver`); the page wraps it with the data fetch (customers + sale-if-edit). The list page's "+ Add Sale" is a `<Link href="/sales/new">`, not a state-driven modal. Form internals (validation, line-item dynamic rows, live subtotal/discount/total math) are unchanged from the Phase 7 modal era — only the surrounding container changed. **Purchases / Casting / Plating keep the previous form-modal pattern** until Phase 10.5 mirrors. Cancel button + `<Link>` back to `/sales` for navigation out without saving.
- **Read-only detail modals (Sales, Phase 10).** `SaleDetailModal` shows data only — no Add Payment / Add Return / Replace Bill / Delete buttons inside. All edits route through: (a) the inline action buttons in the table row (`[$ Pay] [📎 Bill] [↩ Return]`) which open the shared `PaymentActionModal` / `BillActionModal` / `ReturnActionModal`, or (b) the bottom Edit link which navigates to `/sales/[id]/edit`. The Detail modal also renders read-only Payments + Returns history alongside line items. **Purchases / Casting / Plating detail modals still embed `PaymentPanel` / `ReturnPanel`** for now.
