...
`id, date, vendorId (FK → CastingPlatingVendor, nullable, onDelete: SetNull), partyName, partyPhone, discount (BigInt paise, @default(0)), total (BigInt paise, stored), notes, attachmentId (FK → Attachment, unique, nullable, onDelete: SetNull), createdAt, updatedAt, deletedAt`
...
- **Indexes:** `@@index([vendorId])`, `@@index([date])`, `@@index([deletedAt])`, unique on `attachmentId`.
