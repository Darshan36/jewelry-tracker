...
### Bill (built Phase 8, extended Phase 12a)
...
- **Multi-purpose table** (Phase 12a). The same `Bill` table now stores **invoices** (`SALE`, `PURCHASE`, `PURCHASE_PAYMENT`, `CASTING_ENTRY`, `PLATING_ENTRY`) AND **photos** (`PURCHASE_PHOTO`). The distinguishing field is `attachedToType`. Photos and invoices share R2 plumbing, presigned URL flow, soft-delete, status state machine — the only divergence is the client-side MIME allowlist (`PHOTO_MIME_TYPES` excludes PDF) and the cardinality convention (invoices are 1:1 per parent entity; photos are many-to-1). The table name "Bill" is now semantically misleading and is logged in KNOWN_GAPS as a deferred rename candidate.
