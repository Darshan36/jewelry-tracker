...
- **UX migration (Phase 10 + 10.5):** Phase 10 + 10.5 completed the form-as-page + read-only-detail-modal + inline-action-button UX pattern for **Sales and Purchases**. Both entities now have `/<entity>/new` and `/<entity>/[id]/edit` routes; their detail modals are read-only; row Actions columns carry Pay / Bill / Return icon buttons that open the shared `PaymentActionModal` / `BillActionModal` / `ReturnActionModal` (the modals dispatch entity-specific server actions via **prop-injection** — `onSave` / `onAttach` / `onDetach` props supplied by the caller). Sales form pages additionally include an inline pre-upload bill section (file picker → preview → save chains createSale → prepareUpload → R2 PUT → confirmUpload as one transaction). Bill upload preview via browser-native `URL.createObjectURL` (image `<img>`, PDF `<embed>`). **Casting and Plating retain the previous modal-everything pattern** until Phase 10.6 mirrors the work.
- **Hosting:** Vercel
- **Package manager:** npm
...
- **Forms as pages (Sales + Purchases).** Both entities have dedicated `/<entity>/new` and `/<entity>/[id]/edit` routes with standalone form components (`sale-form.tsx`, `purchase-form.tsx`) — RHF + `useFieldArray` + `zodResolver`. The page wraps it with the data fetch (customers/suppliers + entity-if-edit). The list page's "+ Add" is a `<Link>` to `/new`, not a state-driven modal. Form internals (validation, line-item dynamic rows, live subtotal/discount/total math) are unchanged from the Phase 7 modal era — only the surrounding container changed. **Casting and Plating still use the form-modal pattern**; Phase 10.6 migrates them. Cancel button + `<Link>` back to the list page for navigation out without saving.
- **Read-only detail modals (Sales + Purchases).** `SaleDetailModal` / `PurchaseDetailModal` show data only — no Add Payment / Add Return / Replace Bill / Delete buttons inside. All edits route through: (a) the inline action buttons in the table row (`[$ Pay] [📎 Bill] [↩ Return]`) which open the shared `PaymentActionModal` / `BillActionModal` / `ReturnActionModal`, or (b) the bottom Edit link which navigates to `/<entity>/[id]/edit`. The Detail modals also render read-only Payments + Returns history alongside line items. **Casting and Plating detail modals still embed `PaymentPanel`** until Phase 10.6.
- **Inline bill section in form pages (Sales).** The Sales form page includes a pre-upload bill section between line items and totals: file picker → `BillPreview` (browser-native `URL.createObjectURL`). On submit, the chain is `createSale|updateSale → prepareUpload(attachedToType="SALE", attachedToId=savedId) → R2 PUT → confirmUpload` — all as part of the same save transaction. Edit mode adds NEW bills only; replace / remove of an existing bill uses the row-level 📎 action modal (the row-level modal handles the discriminator soft-delete + re-upload flow). On upload failure AFTER entry create, the entry stays saved and an error banner directs the user to the row-level 📎 modal for retry. **Purchases bill-in-form section deferred to Phase 10.6.**
- **Action modal prop-injection dispatch.** `PaymentActionModal` and `ReturnActionModal` accept an `onSave: (data) => Promise<SaveResult>` prop instead of switching on `entityType` internally. `BillActionModal` accepts optional `onAttach: (entityId, billId) => Promise<{ok}>` / `onDetach: (entityId) => Promise<{ok}>` props — used by Casting/Plating to set/clear the entry's `billId @unique` FK; Sales/Purchases pass neither (the discriminator-only attachment is established by `prepareUpload`'s `attachedToType` + `attachedToId` args). The modal's internal chain (`getBillForEntity → softDelete → prepareUpload → R2 PUT → confirmUpload → optional onAttach`) stays entity-agnostic; the caller wires entity-specific actions via prop closure. Phase 10.6 wires casting/plating tables to dispatch the right actions without rewriting the modals. Pattern established Phase 10.5 (`sales-table.tsx`, `purchases-table.tsx`).
