...
- **File storage:** Cloudflare R2 (S3-compatible) for bill / receipt scans (Phase 8) AND purchase photos (Phase 12a). `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner`. Lazy-init wrapper at `src/lib/r2.ts` mirroring the `prisma.ts` Proxy pattern — env is read on first call, not at module load. Browser uploads directly to R2 via 10-min presigned PUT URLs (two-step prepare/confirm flow); server-side downloads issue 1-hr presigned GET URLs. Photos and bills share the same `Attachment` table (renamed from `Bill` in the polish session) and the same R2 infrastructure; the only difference is the `attachedToType` discriminator value (`PURCHASE_PHOTO` vs `PURCHASE`/`SALE`/etc.) and the `PHOTO_MIME_TYPES` allowlist (which excludes PDF). User-facing UI continues to call them "bills" — the rename only affects the internal data layer.
...
### Attachment (built Phase 8 as `Bill`, extended Phase 12a, renamed polish session)
...
- **Originally named `Bill`** in Phase 8 when the table only stored invoice receipts. Phase 12a added photo storage via the `PURCHASE_PHOTO` discriminator; the polish session (sub-phase 3) renamed the model and table to `Attachment` / `attachments` for accurate semantics. **User-facing UI continues to use "bill" / "Add bill" / "View bill" terminology** — the workshop user thinks of these as bills, regardless of the internal name. The rename only affects the data layer.
...
- **Photos vs bills share the same `Attachment` table; discriminator-only dispatch (Phase 12a).** "Bills" (invoices, receipts) and "photos" (visual record attached to a Purchase) both live in the single `Attachment` table. The only difference is `attachedToType` (`PURCHASE` / `SALE` / etc. vs `PURCHASE_PHOTO`) and the client-side MIME allowlist (`PHOTO_MIME_TYPES` excludes PDF). R2 plumbing, prepare/confirm flow, presigned-URL viewing, soft-delete, access control via `attachment-access.ts` ROLE_MATRIX — all polymorphic over `attachedToType`. **Gallery vs single-attachment cardinality is enforced at the application layer**, not the schema: bills are 1:1 per parent (replace soft-deletes the prior bill), photos are many-to-1 per parent (multiple visible thumbnails, individual delete). When adding a new "multi-attachment" feature (e.g., sale photos, vendor evidence photos), extend `ATTACHED_TO_TYPES`, add a `ROLE_MATRIX` entry, mount `<PhotoGallery>` on the parent's form/detail surfaces, and (for the list view) add a `prisma.attachment.groupBy` aggregate in the page query — `<PhotoGallery>` and `<PhotoLightbox>` are entityType-parameterized and reused as-is.
